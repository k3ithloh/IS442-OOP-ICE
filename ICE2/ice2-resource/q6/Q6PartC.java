...
public class Q6PartC {
    ...
    public static String getFarmerDetails(CommunityGarden garden, int plotNumber) {
        ...
        Plot tempPlot = garden.getPlot(plotNumber);
        if (tempPlot == null){
            return "N.A.";
        }
        Farmer tempFarmer = tempPlot.getFarmer();
        if (tempFarmer == null){
            return "available";
        }
        else{
            
            String name = tempFarmer.getName();
            char gender = tempFarmer.getGender();
            String genderFull;
            if (gender =='M'){
                genderFull = "Male";
            }
